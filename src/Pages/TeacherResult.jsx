...
const TeacherResult = () => {
  ...
  const filteredResults = results.filter((res) => {
    ...
    const matchesClass = selectedClass === "All Classes" || res.class === selectedClass;
    const matchesExam = selectedExam === "All Exams" || res.exam === selectedExam;
  ...
  return (
    <>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <h1 className="text-2xl md:text-3xl font-bold text-[#0F6B75]">Results</h1>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div className="p-4 border-b border-gray-200 flex flex-col md:flex-row gap-4 justify-between">
           ...
           <div className="flex gap-3">
            <div className="pr-3 bg-gray-100 rounded-lg cursor-pointer">
              <select 
                value={selectedClass}
                onChange={(e) => setSelectedClass(e.target.value)}
                className="px-4 py-2 text-gray-600 font-medium hover:bg-gray-100 border-none outline-none cursor-pointer"
              >
            ...
            <tbody className="divide-y divide-gray-200">
              {filteredResults.map((res) => (
                <tr key={res.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-center font-bold text-[#0F6B75]">{res.score}</td>
                  <td className="px-6 py-4 text-center">
                    <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                        res.grade.startsWith('A') ? 'bg-green-100 text-green-700' : 
                        res.grade.startsWith('B') ? 'bg-blue-100 text-blue-700' : 'bg-yellow-100 text-yellow-700'
                    }`}>
                      {res.grade}
                    </span>
                  </td>
