  ...
  return (
    <>
      ...
      <div className="md:hidden">
        {exams.map((exam) => (
          <div key={exam.id} className="p-4 border-b border-gray-200 last:border-b-0 hover:bg-gray-50 transition-colors">
            ...
            <div className="flex justify-between items-end">
              <div className="text-sm text-gray-600">
                <p>{exam.date.split(',')[0]}, {exam.date.split(',')[1]}</p>
                <p>{exam.date.split(',')[2]}</p>
              </div>
              
              <div className="flex gap-3 text-gray-500">
                <button className="hover:text-[#0F6B75] transition-colors"><Eye size={18} /></button>
                <button className="hover:text-[#0F6B75] transition-colors"><Edit2 size={18} /></button>
          ...
          <tbody className="divide-y divide-gray-200">
            {exams.map((exam) => (
              <tr key={exam.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-6 py-4 text-center text-gray-600">{exam.questions}</td>
                ...
                <td className="px-6 py-4 text-gray-600 text-sm">
                  {exam.date.split(',')[0]}, {exam.date.split(',')[1]}<br />
                  {exam.date.split(',')[2]}
                </td>
                <td className="px-6 py-4 text-center">
