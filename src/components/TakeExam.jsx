...
import { Clock, ChevronLeft, ChevronRight, CheckCircle, ShieldAlert } from "lucide-react";
...
const TakeExam = ({ exam, onFinish }) => {
  ...
  const [submitError, setSubmitError] = useState(null); // ✅ new
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (exam.attemptId) {
        ...
        if (typeof apiScore === "number") {
          ...
          exam.questionsList.forEach((q) => {
            if (answers[q.id] === q.correctOption) calculatedScore += q.marks;
          });
      ...
      if (status === 403) {
        // Cheating block — most important to surface clearly
        setSubmitError({ type: "cheating", message: backendMessage });
      } else if (status === 400) {
      ...
      <div className="max-w-2xl mx-auto bg-white p-8 rounded-2xl border border-gray-200 text-center space-y-6">
        ...
        <div className="bg-gray-50 p-6 rounded-xl inline-block w-full">
          ...
          <p className="text-4xl font-bold text-[#0F6B75]">
            {score}{" "}
            <span className="text-lg text-gray-400 font-medium">/ {exam.totalMarks}</span>
          </p>
        </div>
        <button
          onClick={onFinish}
    ...
    <div className="flex flex-col h-full max-w-4xl mx-auto">
      ...
      <div className="bg-white p-4 rounded-xl border border-gray-200 flex justify-between items-center mb-6 shadow-sm">
        ...
        <div className={`flex items-center gap-2 font-mono text-xl font-bold px-4 py-2 rounded-lg ${
          timeLeft < 300 ? "bg-red-50 text-red-600" : "bg-teal-50 text-[#0F6B75]"
        }`}>
          <Clock size={20} />
          {formatTime(timeLeft)}
        </div>
      ...
      {/* ✅ Submit error banner — shown above question area */}
      {submitError && (
        <div className={`flex items-start gap-3 px-4 py-3.5 rounded-xl border mb-4 ${
          ...
          <div>
            <p className="font-semibold text-sm">
              {submitError.type === "cheating" ? "Submission Blocked" : "Submission Failed"}
            </p>
          ...
          <div className="bg-white p-8 rounded-2xl border border-gray-200 shadow-sm min-h-[400px] flex flex-col">
            <div className="flex justify-between items-start mb-6">
              <span className="bg-gray-100 text-gray-600 px-3 py-1 rounded-full text-sm font-bold">
                Question {currentQuestionIndex + 1} of {exam.questionsList.length}
              </span>
          ...
          <div className="flex justify-between items-center">
            ...
            {currentQuestionIndex === exam.questionsList.length - 1 ? (
              <button
                ...
                disabled={submitting}
                className={`flex items-center gap-2 px-8 py-3 rounded-xl font-bold text-white shadow-lg transition-all hover:scale-105 ${
                  submitting
                    ? "bg-gray-400 cursor-not-allowed hover:scale-100"
                    : "bg-[#0F6B75] hover:bg-[#0c565e] shadow-teal-700/20"
                }`}
              >
                {submitting ? (
                  <>
